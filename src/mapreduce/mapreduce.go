...
type MapReduce struct {
	...
	JobListMap     *list.List
	JobListReduce  *list.List
	doneMap        bool
	doneReduce     bool
}
...
func InitMapReduce(nmap int, nreduce int,
	...
	mr.MapState = make([]string, nmap, nmap)
	mr.ReduceState = make([][]string, nmap)
	for i := range mr.ReduceState {
		mr.ReduceState[i] = make([]string, nreduce)
	}
	mr.DoneMapChan = make(chan bool)
	mr.DoneReduceChan = make(chan bool)
	mr.doneMap = false
	mr.doneReduce = false
	// initialize job list
	mr.JobListMap = list.New()
	for jobnumber := 0; jobnumber < mr.nMap; jobnumber++ {
		args := &DoJobArgs{}
		args.File = mr.file
		args.Operation = Map
		args.JobNumber = jobnumber
		args.NumOtherPhase = mr.nReduce
		mr.JobListMap.PushBack(args)
	}
	mr.JobListReduce = list.New()
	for jobnumber := 0; jobnumber < mr.nReduce; jobnumber++ {
		args := &DoJobArgs{}
		args.File = mr.file
		args.Operation = Reduce
		args.JobNumber = jobnumber
		args.NumOtherPhase = mr.nMap
		mr.JobListReduce.PushBack(args)
	}
	return mr
}
...
func (mr *MapReduce) StartRegistrationServer() {
	rpcs := rpc.NewServer()
	rpcs.Register(mr)
	os.Remove(mr.MasterAddress) // only needed for "unix"i
